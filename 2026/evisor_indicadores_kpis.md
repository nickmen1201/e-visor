...
| **HU** — Horas de utilización | `HU [h] = Σ(activeenergyimportday [Wh]) / máximo(activepower [W])` · El cociente Wh/W resuelve a horas directamente, sin constante de conversión. Identidad de cierre: `HU = LF · T`, con T = horas del período (24 para resolución diaria) — permite verificación cruzada con LF. | Número de horas equivalentes al nivel de máxima potencia necesarias para acumular el consumo total del período. Conecta la dimensión de energía con la de potencia. | `activeenergyimportday`, `activepower` | Medidores Landis |
...
| **THD-V** — Distorsión armónica total de voltaje | `THD_V = relativethdvoltage` (variable directa del medidor). Para diagnóstico por fase: `harmonicsv1`, `harmonicsv2`, `harmonicsv3`. | Mide el contenido armónico de la tensión. Valores elevados indican perturbaciones que degradan equipos sensibles, aumentan pérdidas y pueden invalidar mediciones. Umbral normal < 5 % en sistemas BT (≤ 1 kV). | `relativethdvoltage` [%] · diagnóstico: `harmonicsv1`, `harmonicsv2`, `harmonicsv3` [%] | Medidores Landis |
...
- **Factor de emisión CO₂:** El valor correcto y vigente es **0,097018 tCO₂e/MWh** (XM S.A. E.S.P., publicado el 30 de enero de 2026 para el año 2025).
- **Datos pendientes de fuentes externas:** KPI 02 requiere `N_usuarios_activos` de sistemas académicos y RRHH. KPI 06 requiere confirmar la resolución temporal de los archivos Fronius antes de calcular.
